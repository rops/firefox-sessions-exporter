...
solution that can allow the user activities to be protected in this kind of situations and for
...
the current session. The infrastructure allows the user to retrieve
...
several aspects of a mobile device and adapts itself to the surrounding
...
that this power-saving switching behavior is completely transparent
...
the user shall decide to save her session and if necessary send it to
another device where to continue browsing the web without any additional
effort.\\ \\
\textbf{-- oppure x quanto riguarda la migration -- }\\ \\
In addition to these problematic situations, we also aimed to provide a solution for \emph{another kind of scenario}, that is whenever the user may decide to move her activity to a different device and continue browsing without any additional effort. Unfortunately existing solutions do not allow to completely migrate a browsing session in such a transparent way that the user can resume her task like nothing changed. \\ \\
\textbf{-------- }\\ \\
Given that, our \emph{goal} was to guarantee a better user experience to support the defined scenarios , allowing the user to restore and preserve her browsing session
as connection faults occur and to design a smart infrastructure to support session migration such that no effort are required to the user to complete the task.
...
add-on. This choice had all the features required to retrieve all the
...
to have lack of portability and compatibility, since every browser needs
...
out to a wide target of devices as well. \\
...
Our target browser has been the popular
...
add-ons in their current mobile versions), providing one of the most
complete and advanced SDK for add-ons development. We also preferred
...
and adapted to our needs to parse the whole document and fix the
...
as SwishApp, which allows to transfer datas between near devices. The
interaction between our extension and the SwishApp is detailed in
section ~\ref{testswish}.\\
...
To test the adaptativity of our system with respect to the session migration
we exploited the Swish application to move a session from a device
...
the default handler for .session file ( custom extension we defined
for our session files ), just moves the received file to the sessions
directory and opens Firefox with a special URL, calling back the add-ons
...
proved with ~\ref{testdrop} test case.
